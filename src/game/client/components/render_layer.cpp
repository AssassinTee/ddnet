...
bool CRenderLayerGroup::DoRender(const CRenderLayerParams &Params) const
{
	if(!g_Config.m_GfxNoclip || Params.m_RenderType == CMapLayers::TYPE_FULL_DESIGN)
	{
		...
		if(m_pGroup->m_Version >= 2 && m_pGroup->m_UseClipping)
		{
			// set clipping
			float aPoints[4];
			RenderMap()->MapScreenToInterface(Params.m_Center.x, Params.m_Center.y, Params.m_Zoom);
...
{
	RenderMap()->MapScreenToGroup(Params.m_Center.x, Params.m_Center.y, m_pGroup, Params.m_Zoom);
}
