...
void CDebugHud::RenderTuning()
{
	...
	float y = 27.0f;
	int Count = 0;
	for(int i = 0; i < StandardTuning.Num(); i++)
	{
		char aBuf[128];
		...
		x += 5.0f;
		TextRender()->Text(0x0, x, y + Count * 6, 5, CTuneParams::ms_apNames[i], -1.0f);
	...
	CTuneParams *ClinetTuning = &m_pClient->m_Tuning[g_Config.m_ClDummy];
