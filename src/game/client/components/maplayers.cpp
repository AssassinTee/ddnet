...
void CMapLayers::OnMapLoad()
{
	...
	if(!m_vTotalLayerCount.empty())
		m_vTotalLayerCount.clear();
	if(!m_vOverlayCount.empty())
		m_vOverlayCount.clear();

	m_vTotalLayerCount.resize(m_pLayers->NumGroups());
	m_vOverlayCount.resize(m_pLayers->NumGroups());
	...
	int TotalTileLayerCount = 0;
	int TotalQuadLayerCount = 0;
	...
	for(int g = 0; g < m_pLayers->NumGroups(); g++)
	{
		...
		std::vector<int> vTotalLayerCounter(pGroup->m_NumLayers, 0);
		std::vector<int> vOverlayCounter(pGroup->m_NumLayers, 0);
		...
		for(int l = 0; l < pGroup->m_NumLayers; l++)
		{
			...
			if(m_Type <= TYPE_BACKGROUND_FORCE)
			{
				if(PassedGameLayer)
				{
					m_vTotalLayerCount[g] = vTotalLayerCounter;
					m_vOverlayCount[g] = vOverlayCounter;
					return;
				}
			}
			...
			if(pLayer->m_Type == LAYERTYPE_TILES && Graphics()->IsTileBufferingEnabled())
			{
				...
				{
					vTotalLayerCounter[l] = TotalTileLayerCount;
					vOverlayCounter[l] = TileLayerAndOverlayCount - 1;
					TotalTileLayerCount += TileLayerAndOverlayCount;
					int CurOverlay = 0;
			...
			{
				vTotalLayerCounter[l] = TotalQuadLayerCount;
				TotalQuadLayerCount += 1;
...
void CMapLayers::RenderTileLayer(int LayerIndex, const ColorRGBA &Color) const
{
...
void CMapLayers::RenderTileBorder(int LayerIndex, const ColorRGBA &Color, int BorderX0, int BorderY0, int BorderX1, int BorderY1) const
{
	STileLayerVisuals &Visuals = *m_vpTileLayerVisuals[LayerIndex];
...
void CMapLayers::RenderKillTileBorder(int LayerIndex, const ColorRGBA &Color) const
{
	STileLayerVisuals &Visuals = *m_vpTileLayerVisuals[LayerIndex];
	if(Visuals.m_BufferContainerIndex == -1)
		return; // no visuals were created
...
void CMapLayers::OnRender()
{
	...
	bool ShowEntities = EntityOverlayVal > 0;
	const ColorRGBA EntityColor = ColorRGBA(1.0f, 1.0f, 1.0f, EntityOverlayVal / 100.0f);
	int GroupStart = m_Type == TYPE_FOREGROUND ? m_GameLayerGroup : 0;
	int GroupEnd = (m_Type == TYPE_BACKGROUND || m_Type == TYPE_BACKGROUND_FORCE) ? std::min(m_GameLayerGroup + 1, m_pLayers->NumGroups()) : m_pLayers->NumGroups();
	...
	for(int g = GroupStart; g < GroupEnd; g++)
	{
		...
		if((!g_Config.m_GfxNoclip || m_Type == TYPE_FULL_DESIGN) && pGroup->m_Version >= 2 && pGroup->m_UseClipping)
		{
			...
			if(x1 < 0.0f || x0 > 1.0f || y1 < 0.0f || y0 > 1.0f)
			{
				...
				//LayersOfGroupCount(pGroup, QuadLayerCounter, PassedGameLayer);
				continue;
			}
		...
		for(int l = 0; l < pGroup->m_NumLayers; l++)
		{
			...
			switch(m_Type) {
				case TYPE_BACKGROUND_FORCE:
					if((pLayer->m_Type == LAYERTYPE_TILES && !g_Config.m_ClBackgroundShowTilesLayers) || IsEntityLayer)
						continue;
			...
			if((OnlyShowEntities && !IsEntityLayer) || (!ShowEntities && IsEntityLayer))
				continue;

			if(pLayer->m_Type == LAYERTYPE_TILES && Input()->ModifierIsPressed() && Input()->ShiftIsPressed() && Input()->KeyPress(KEY_KP_0))
				WriteTilesetDebugDump((CMapItemLayerTilemap *)pLayer, g, l);
			...
			/*dbg_msg("debug", "Type %d", m_Type);
			dbg_msg("debug", "LayerType: %d, PassedGameLayer: %d", LayerType, PassedGameLayer);
			dbg_msg("debug", "vCountSize: %d, group: %d, NumGroups: %d", m_vTotalLayerCount.size(), g, m_pLayers->NumGroups());
			dbg_msg("debug", "vCountSizeGroup: %d, layer: %d", m_vTotalLayerCount[g].size(), l);
			dbg_msg("debug", "Count: %d, overlays: %d", m_vTotalLayerCount[g][l], m_vOverlayCount[g][l]);*/
			...
			if(pLayer->m_Type == LAYERTYPE_TILES)
			{
				if(!IsEntityLayer)
				{
					...
					if(pLayerTilemap->m_Image >= 0 && pLayerTilemap->m_Image < m_pImages->Num())
						Graphics()->TextureSet(m_pImages->Get(pLayerTilemap->m_Image));
					else
						Graphics()->TextureClear();

					ColorRGBA Color = ColorRGBA(pLayerTilemap->m_Color.r / 255.0f, pLayerTilemap->m_Color.g / 255.0f, pLayerTilemap->m_Color.b / 255.0f, pLayerTilemap->m_Color.a / 255.0f);
					...
					/*if(!Graphics()->IsTileBufferingEnabled())
						RenderTilelayerNoTileBuffer(LayerType, pTilesData, pLayerTilemap, Color);
					else*/
					RenderTilelayerWithTileBuffer(LayerType, m_vTotalLayerCount[g][l], m_vOverlayCount[g][l], Color);
				}
				else
				{
					/*if(!Graphics()->IsTileBufferingEnabled())
					...
					RenderTilelayerWithTileBuffer(LayerType, m_vTotalLayerCount[g][l], m_vOverlayCount[g][l], EntityColor);
				}
			}
			else if(pLayer->m_Type == LAYERTYPE_QUADS)
			{
				...
				if(m_Type == TYPE_BACKGROUND_FORCE || m_Type == TYPE_FULL_DESIGN)
				{
					if(g_Config.m_ClShowQuads || m_Type == TYPE_FULL_DESIGN)
					{
						...
						{
							RenderQuadLayer(m_vTotalLayerCount[g][l], pLayerQuads, true);
						}
					}
				}
				else
				{
					...
					{
						RenderQuadLayer(m_vTotalLayerCount[g][l], pLayerQuads, false);
					}
				}
			}
		}
...
void CMapLayers::RenderTilelayerWithTileBuffer(int LayerType, int TotalLayerCount, int OverlayCount, const ColorRGBA &Color) const
{
	if(LayerType == -1)
	{
		Graphics()->BlendNormal();

		// Render default tileset
		RenderTileLayer(TotalLayerCount, Color);
		return;
	}

	if(LayerType != LAYER_SWITCH && LayerType != LAYER_SPEEDUP)
	{
		Graphics()->TextureSet(m_pImages->GetEntities(MAP_IMAGE_ENTITY_LAYER_TYPE_ALL_EXCEPT_SWITCH));
		Graphics()->BlendNormal();
		if(LayerType == LAYER_GAME)
			RenderKillTileBorder(TotalLayerCount, Color.Multiply(GetDeathBorderColor()));
	}
	else if(LayerType == LAYER_SWITCH)
	{
	...
	if(!g_Config.m_ClTextEntities || !OverlayCount)
		return;

	if(LayerType == LAYER_SPEEDUP)
		Graphics()->WrapNormal();

	auto GetOverlayTexture = [&](int Lt, int OverlayId){
		if(Lt == LAYER_TELE)
			return m_pImages->GetOverlayCenter();
		else if((OverlayId == 0 && Lt == LAYER_SPEEDUP) || (OverlayId == 1 && Lt == LAYER_SWITCH))
			return m_pImages->GetOverlayBottom();
		else
			return m_pImages->GetOverlayTop();
	};

	for(int i = 0; i < OverlayCount; ++i)
	{
		Graphics()->TextureSet(GetOverlayTexture(LayerType, i));
		RenderTileLayer(TotalLayerCount + i + 1, Color);
	}
}

void CMapLayers::RenderTilelayerNoTileBuffer(int LayerType, void *pTilesData, CMapItemLayerTilemap *pLayerTilemap, const ColorRGBA &Color)
{
	if(LayerType != LAYER_SWITCH && LayerType != LAYER_SPEEDUP && LayerType != -1)
		Graphics()->TextureSet(m_pImages->GetEntities(MAP_IMAGE_ENTITY_LAYER_TYPE_ALL_EXCEPT_SWITCH));
...
ColorRGBA CMapLayers::GetDeathBorderColor() const
{
	...
	double Alpha = 0.3 + 0.35 * (1.0 + std::sin(2.0 * (double)pi * Seconds / 3.0));
	return ColorRGBA(1.f, 1.f, 1.f, (float)Alpha);
}

void CMapLayers::WriteTilesetDebugDump(const CMapItemLayerTilemap *pLayerTilemap, int Group, int Layer) const
{
	CTile *pTiles = (CTile *)m_pLayers->Map()->GetData(pLayerTilemap->m_Data);
	CServerInfo CurrentServerInfo;
	Client()->GetServerInfo(&CurrentServerInfo);
	char aFilename[IO_MAX_PATH_LENGTH];
	str_format(aFilename, sizeof(aFilename), "dumps/tilelayer_dump_%s-%d-%d-%dx%d.txt", CurrentServerInfo.m_aMap, Group, Layer, pLayerTilemap->m_Width, pLayerTilemap->m_Height);
	IOHANDLE File = Storage()->OpenFile(aFilename, IOFLAG_WRITE, IStorage::TYPE_SAVE);
	if(File)
	{
		for(int y = 0; y < pLayerTilemap->m_Height; y++)
		{
			for(int x = 0; x < pLayerTilemap->m_Width; x++)
				io_write(File, &(pTiles[y * pLayerTilemap->m_Width + x].m_Index), sizeof(pTiles[y * pLayerTilemap->m_Width + x].m_Index));
