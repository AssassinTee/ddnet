	...
	CEntity(pGameWorld, CGameWorld::ENTTYPE_PICKUP, vec2(0, 0), gs_PickupPhysSize)
{
	...
	m_Number = Number;
	m_Flags = Flags;
	std::fill_n(m_SpawnTick, MAX_CLIENTS, -1);
...
void CPickup::Tick()
{
	...
	for(int i = 0; i < Num; ++i)
	{
		...
		{
			if(g_Config.m_SvHealthAndAmmo && m_SpawnTick[pChr->GetPlayer()->GetCid()] != -1)
				continue;
...
{
	if(NetworkClipped(SnappingClient) || (SnappingClient >= 0 && m_SpawnTick[SnappingClient] != -1 && g_Config.m_SvHealthAndAmmo))
		return;
