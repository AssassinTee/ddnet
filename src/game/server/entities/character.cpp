	...
	if(GameServer()->IsUniqueRace() && g_Config.m_SvFastcap)
	{
		...
		if(d)
		{
			...
			for(int i = 0; i < End; i++)
			{
				...
				if(!m_GotFastcapFlag1 && distance(Pos, GameServer()->m_pController->m_FastcapFlag1) < 14 + GetProximityRadius())
				{
				...
				if(!m_GotFastcapFlag2 && distance(Pos, GameServer()->m_pController->m_FastcapFlag2) < 14 + GetProximityRadius())
				{
