...
void CRenderLayerQuads::RenderQuadLayer(float Alpha, const CRenderLayerParams &Params)
{
	...
	if(Params.m_DebugRenderClusterClips)
	{
		for(auto &QuadCluster : m_vQuadClusters)
		{
			if(!ShowClipped(QuadCluster.m_ClipRegion) || !QuadCluster.m_ClipRegion.has_value())
				continue;
...
bool CRenderLayerQuads::ShowClipped(const std::optional<CClipRegion> &ClipRegion)
{
