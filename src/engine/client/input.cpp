...
bool CInput::OpenJoystick(SDL_JoystickID JoystickID)
{
...
CInput::CJoystick::CJoystick(CInput *pInput, int Index, SDL_Joystick *pDelegate)
{
	...
	SDL_GUID GUID = SDL_GetJoystickGUID(pDelegate);
	SDL_GUIDToString(GUID, m_aGUID, sizeof(m_aGUID));
...
{
	if(!m_MouseFocus || !m_InputGrabbed || !g_Config.m_InpControllerEnable)
		return false;
...
static int TranslateKeyEventKey(const SDL_KeyboardEvent &KeyEvent)
{
	...
	int Key = g_Config.m_InpTranslatedKeys ? SDL_GetScancodeFromKey(KeyEvent.key, NULL) : KeyEvent.scancode;
...
int CInput::Update()
{
	...
	while(SDL_PollEvent(&Event))
	{
		...
		case SDL_EVENT_WINDOW_DISPLAY_CHANGED:
			...
			{
			case SDL_EVENT_WINDOW_DISPLAY_CHANGED:
				Graphics()->SwitchWindowScreen(Event.window.data1, false);
				break;
			case SDL_EVENT_WINDOW_MOVED:
